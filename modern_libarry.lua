...
-- library init
    ...
    local themes = {
        preset = {
            ["outline"] = rgb(20, 20, 30), -- 
            ["inline"] = rgb(50, 50, 70), --
            ["accent"] = rgb(140, 90, 255), --
            ["contrast"] = rgb(45, 45, 60),
...
-- Tambahkan animasi fade-in dan fade-out untuk elemen UI
function library:fadeIn(element, duration)
    local tween = tween_service:Create(element, TweenInfo.new(duration or 0.5, Enum.EasingStyle.Quad), {Transparency = 0})
    tween:Play()
end

function library:fadeOut(element, duration)
    local tween = tween_service:Create(element, TweenInfo.new(duration or 0.5, Enum.EasingStyle.Quad), {Transparency = 1})
    tween:Play()
end

-- Animasi hover pada tombol
function library:addHoverEffect(button)
    button.MouseEnter:Connect(function()
        tween_service:Create(button, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(160, 110, 255)}):Play()
    end)
    button.MouseLeave:Connect(function()
        tween_service:Create(button, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(140, 90, 255)}):Play()
    end)
end

-- Terapkan efek hover pada semua tombol UI
for _, instance in pairs(library.instances) do
    if instance:IsA("TextButton") then
        library:addHoverEffect(instance)
    end
end
